function SearchFormView({onSearch, onType, onText, summaryNav}) {
      ...
      <div class="search-container input-group p-3 m-2">
        ...
        <input type="search" class="form-control rounded" placeholder="pizza" onChange={e => onText(e.target.value) }></input>
          ...
          <div class="input-group-append">
              ...
              <button class="btn btn-outline-success" onClick={e=>summaryNav.code()}>{summaryNav.label}</button>
          </div>
      </div>
  );
}

function SearchResultView({searchResults, dishChosen, detailsNav}){
  return (
